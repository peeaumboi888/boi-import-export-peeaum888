...
export default function ExportsPage() {
  ...
  const loadTransactions = async () => {
    try {
      const data = await api.importExport.getTransactions("export");
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "ประเทศปลายทาง",
      render: (item: any) => <span>{item.destinationCountry}</span>,
    },
    {
      key: "quantity",
      header: "จำนวน",
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">รายการส่งออก</h1>
        <p className="text-muted-foreground">รายการส่งออกสินค้าทั้งหมด</p>
      </div>

      <SearchBar placeholder="ค้นหารายการส่งออก..." onSearch={handleSearch} />
